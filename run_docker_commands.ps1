if (docker ps -a --filter "name=mysql_hometask_5" --format "{{.Names}}" | Select-String "mysql_hometask_5") {
    Write-Host "Stopping and removing existing container..."
    docker stop mysql_hometask_5 | Out-Null
    docker rm mysql_hometask_5 | Out-Null
}

Write-Host "Starting a new MySQL container..."
docker run -d --name mysql_hometask_5 `
...
cmd /c "docker exec -i mysql_hometask_5 mysql -u root -p11111<.\db_init.sql"
